...
window.SwipeSensor001 = function () {
	...
	const Evt            = window.Evt || {
		add: function (el, evt, handler, bubbles) {
			if (el.addEventListener) el.addEventListener(evt, handler, bubbles || false);
		...
		rm: function (c, b, a) {
			if (c.detachEvent) {
				...
				c[b + a] = null
			} else c.removeEventListener(b, a, false);
		},
	...
	this.powerUp = function (triggerEl, callBack) {
		...
		console.log(`%c${NAME} > powerUp`, console__style001);
	};
	...
	function addListeners(el) {
		...
		try {
			...
			console.warn(`%c${NAME} > addListeners > Error: ${err}`, console__style001);
		}
	...
	function touchStart(evt) {
		...
		// evt.preventDefault();
		fingerCount = evt.touches.length; // Get the total number of fingers touching the screen
		// Since we're looking for a swipe (single finger) and not a gesture (multiple fingers), verify only one finger was used
		if (fingerCount === 1) {
		...
		} else touchCancel(evt); // More than one finger touched so cancel
	}
