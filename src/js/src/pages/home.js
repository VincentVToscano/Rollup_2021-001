...
window.app = function () {
	 ...
	 */
	function boot() {
		console.log(`%c ${NAME} > ${PAGE_NAME} > boot`, console__style001);
		init();
	}
	 ...
	 */
	function init() {
		console.log(`%c ${NAME} > ${PAGE_NAME} > init`, console__style001);
		app.test_emit                             = new TestEmitter('BROADCASTER_001', 400);
		id('content_aside').style.backgroundColor = 'pink';
		swipe.powerUp(id('hero'));
		addHandlers();
	}

	function addHandlers() {
		Evts.addMultiple(id('hero'), ['mouseover', 'mouseout'], onContact, true);
	...
	function onSwipe(evt) {
		console.log(`%c ${NAME} > ${PAGE_NAME} > onSwipe`, console__style001);

		switch (evt.type) {
			case swipe.EVT_ON_SWIPE_LEFT:
				console.log(`%c ${NAME} > ${PAGE_NAME} > onSwipe > ${evt.type}`, console__style001);
				break;
